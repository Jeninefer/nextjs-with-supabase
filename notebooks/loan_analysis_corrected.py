...
def calculate_monthly_transfers_operations(df, loan_id_col, date_col):
    ...
    # Aggregate by month
    monthly = df_unique.groupby('month').agg(
    ...
    monthly['avg_ops_per_transfer'] = (
        monthly['operations'] / monthly['transfers']
    ).round(2)
...
def calculate_average_amounts(df, loan_id_col, amount_col, date_col):
    ...
    avg_per_operation = (
        df_valid.groupby('month')['amount']
        .mean()
        .rename('avg_amount_per_operation')
    )
    ...
    avg_per_transfer = (
        transfer_totals.groupby('month')['transfer_total']
        .mean()
        .rename('avg_amount_per_transfer')
    )
...
def calculate_outstanding_balance(
    ...
    outstanding = (
        cum_disbursements - cum_principal_paid - cum_writeoffs
    ).clip(lower=0)
...
def validate_outstanding_calculation(result_df):
    ...
    checks['disbursements_monotonic'] = (
        result_df['cum_disbursements'].diff().fillna(0) >= 0
    ).all()
    ...
    checks['final_outstanding_reasonable'] = (
        0 <= final_outstanding <= total_disbursed
    )
