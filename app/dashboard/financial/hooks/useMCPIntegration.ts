...
const mockMCPClient = {
  initializeServer: async (
    ...
    const envSummary = env && Object.keys(env).length > 0 ? ` with env [${Object.keys(env).join(', ')}]` : '';
  ...
  storeMemory: async (key: string, payload?: unknown) => ({
    success: true,
    data: `Stored ${key}${payload !== undefined ? `: ${JSON.stringify(payload)}` : ''}`
  }),
