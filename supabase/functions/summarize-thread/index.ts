...
const isSenderRecord = (obj: unknown): obj is SenderRecord => {
  return (
    ...
    (typeof (obj as { display_name?: unknown }).display_name === "string" || (obj as { display_name?: unknown }).display_name == null)
  );
};

const resolveSender = (sender: MessageRow["sender"]): SenderRecord | null => {
  let resolved: SenderRecord | SenderRecord[] | null | undefined = sender;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an assistant for a financial intelligence platform. Summarize chat threads related to market analysis and insights. Return 3-5 concise bullet points using dashes (\"- \"). If there are any action items, list them at the end under a separate heading \"Action Items:\", each as a dash. If there are no action items, omit the \"Action Items:\" section.",
          },
          {
            role: "user",
