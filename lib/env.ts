class MissingEnvironmentVariableError extends Error {
  constructor(variableNames: readonly string[]) {
    ...
    super(`Missing required environment variable${variableNames.length > 1 ? "s" : ""}: ${formattedNames}`);
...
const { NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_PUBLISHABLE_OR_ANON_KEY } = readSupabaseEnv();
